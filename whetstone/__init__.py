...
class Whetstone:
    ...
    def _request(self, session_type, method, path, params={}, body=None):
        ...
        try:
            ...
            print(f"{response_json['name']} {response_json['code']}\n{response_json['message']}")
            print(e)
            raise e
    ...
    def authorize_client(self, **kwargs):
        ...
        if access_token:
            ...
            if expires_at > now:
                ...
                self.client_session.headers['Authorization'] = f"Bearer {access_token.get('access_token')}"
                return "Authorized!"
            else:
        ...
        if isinstance(client_credentials, tuple):
            ...
            self.client_session.headers['Authorization'] = f"Bearer {token.get('access_token')}"
            return "Authorized!"
        else:
    ...
    def get(self, schema, record_id=None, params={}, session_type='client'):
        ...
        elif schema == 'generic-tags' or session_type == 'frontend':
            response = self._request(
                method='GET',
                session_type=session_type,
                path=schema,
